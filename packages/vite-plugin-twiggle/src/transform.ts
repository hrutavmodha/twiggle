        ...
        visitor: {
            JSXExpressionContainer(path) {
                ...
                if (t.isCallExpression(expr) && t.isMemberExpression(expr.callee)) {
                    ...
                    if (t.isIdentifier(property) && property.name === 'map') {
                        ...
                        if (
                            ...
                            expr.arguments[0] = t.arrowFunctionExpression(
                                callback.params,
                                t.callExpression(t.identifier('runSideEffect'), [
                                    t.arrowFunctionExpression([], callback.body),
                                ])
                            )
                        }
                    }
            ...
            CallExpression(path) {
                ...
                if (t.isIdentifier(callee) && (callee.name === 'jsxDEV' || callee.name === 'jsx')) {
                    ...
                    t.isMemberExpression(callee) &&
                    t.isIdentifier(callee.property) &&
                    (callee.property.name === 'jsxDEV' || callee.property.name === 'jsx')
                ) {
                    isJsxCall = true
                }
                            ...
                            if (isStateGetCall(children)) {
                                ...
                                children.elements.forEach((child, index) => {
                                    if (isStateGetCall(child)) {
                                        children.elements[index] = t.arrowFunctionExpression(
                                            [],
                                            child as any
                                        )
                                    }
